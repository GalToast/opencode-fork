...
:::tip
`websearch` supports backend routing via env vars:

- `OPENCODE_WEBSEARCH_BACKEND=auto|exa|searxng` (default: `auto`)
- `OPENCODE_SEARXNG_URL=http://127.0.0.1:8080` (for local/self-hosted SearXNG)
- `OPENCODE_EXA_API_KEY=...` (optional Exa key to avoid free-tier limits)

In `auto` mode, OpenCode tries Exa first and can fall back to SearXNG if configured.
:::
