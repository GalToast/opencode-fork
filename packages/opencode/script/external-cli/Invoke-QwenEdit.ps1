param(
  ...
  [string] $McpConfigPath = "script\external-cli\mcp-configs\empty-mcp.json"
)
...
$resolvedMcpConfigPath = (Resolve-Path -LiteralPath (Join-Path $repo $McpConfigPath)).Path
...
$qwenArgs.Add($resolvedMcpConfigPath)
$qwenArgs.Add("--approval-mode")
$qwenArgs.Add($ApprovalMode)
$qwenArgs.Add("--output-format")
$qwenArgs.Add($OutputFormat)
if ($Model) {
  $qwenArgs.Add("--model")
  $qwenArgs.Add($Model)
}
