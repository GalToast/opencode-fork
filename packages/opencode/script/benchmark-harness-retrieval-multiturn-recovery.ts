...
import {
  ...
  runRetrievalMultiturnRecoveryBenchmark,
...
async function withWorkspace<T>(fn: (prepareWorkspace: (scenarioID: string) => Promise<string>) => Promise<T>) {
  const root = await mkdtemp(path.join(os.tmpdir(), "opencode-retrieval-multiturn-recovery-"))
...
const profile = classifyRetrievalMultiturnRecoveryLift(result)

if (JSON_OUTPUT) {
  console.log(JSON.stringify({ ...result, profile, recommended_use: retrievalMultiturnRecoveryPolicyHint(profile) }, null, 2))
} else {
  console.log("Retrieval multi-turn recovery benchmark")
  ...
  console.log(`recommended_use: ${retrievalMultiturnRecoveryPolicyHint(profile)}`)
  console.table(
