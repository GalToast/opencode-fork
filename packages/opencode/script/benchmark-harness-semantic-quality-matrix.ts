...
import {
  ...
  runSemanticQualityBenchmark,
  semanticQualityPolicyHint,
  summarizeSemanticQualityLiftProfiles,
  type SemanticQualityBenchmarkResult,
...
const MODEL_POOL = process.env.HARNESS_BENCH_MODEL_POOL ?? "opencode-free"
...
async function discoverModels() {
  if (MODEL_POOL === "alibaba-coding-plan") {
    const discovered = await Provider.listAlibabaCodingPlanSubagentModels()
...
const instanceRoot = await mkdtemp(path.join(os.tmpdir(), "opencode-semantic-quality-matrix-instance-"))
const benchmarkRoot = await mkdtemp(path.join(os.tmpdir(), "opencode-semantic-quality-matrix-"))

try {
  const results: SemanticQualityBenchmarkResult[] = []
    ...
    fn: async () => {
      ...
      if (!CHILD_MODE && selected.length > 1) {
        for (const model of selected) {
          progressLine(`Spawning isolated semantic quality lift child for ${model.providerID}/${model.modelID}`)
          progressJSON({ type: "model_spawn", providerID: model.providerID, modelID: model.modelID, pool: MODEL_POOL })
          const child = Bun.spawn(
            [process.execPath, "run", "script/benchmark-harness-semantic-quality-matrix.ts"],
            {
              cwd: process.cwd(),
              env: {
                ...process.env,
            ...
            await Promise.race([
              ...
              Bun.sleep(CHILD_GRACE_MS).then(() => {
                if (resolvedExitCode === null) {
                  progressJSON({ type: "child_forced_close_after_result", providerID: model.providerID, modelID: model.modelID, graceMS: CHILD_GRACE_MS })
                  child.kill()
                }
              }),
            ])
          ...
          if (!childFinalJSON && exitCode !== 0) {
            ...
            progressLine(`Failed semantic quality lift for ${model.providerID}/${model.modelID}: ${message}`)
          ...
          try {
            ...
            const parsed = JSON.parse(childFinalJSON) as {
              results?: SemanticQualityBenchmarkResult[]
            ...
            if (childResult) {
              progressLine(
                `Completed semantic quality lift for ${model.providerID}/${model.modelID}: quality_lift=${childResult.qualityLift}, semantic_accuracy=${Math.round(childResult.semanticAccuracy * 100)}%`,
              )
              progressJSON({
                type: "model_complete",
      ...
      for (const model of selected) {
        progressLine(`Running semantic quality lift for ${model.providerID}/${model.modelID}`)
        ...
        try {
          const result = await runSemanticQualityBenchmark({
          ...
          results.push(result)
          progressLine(
            `Completed semantic quality lift for ${model.providerID}/${model.modelID}: quality_lift=${result.qualityLift}, semantic_accuracy=${Math.round(result.semanticAccuracy * 100)}%`,
          )
          progressJSON({
            type: "model_complete",
            providerID: model.providerID,
  ...
  const profileSummary = summarizeSemanticQualityLiftProfiles(ranked)

  if (JSON_OUTPUT) {
    console.log(
      JSON.stringify({
        suite: "semantic_quality_lift_matrix",
    ...
    console.log("Semantic quality lift matrix")
    ...
    console.table(
      ranked.map((result) => ({
        ...
        profile: classifySemanticQualityLift(result),
        recommended_use: semanticQualityPolicyHint(classifySemanticQualityLift(result)),
        reasoning_lift: result.looseQualityLift,
        control_lift: result.qualityLift,
        ...
        strict_correct: `${result.semanticCorrectCount}/${result.scenarioCount}`,
        strict_baseline: `${result.baselineCorrectCount}/${result.scenarioCount}`,
        loose_correct: `${result.semanticLooseCorrectCount}/${result.scenarioCount}`,
        loose_baseline: `${result.baselineLooseCorrectCount}/${result.scenarioCount}`,
        routing_reasoning_lift: result.categorySummary.find((item) => item.category === "routing")?.looseQualityLift ?? 0,
        routing_control_lift: result.categorySummary.find((item) => item.category === "routing")?.qualityLift ?? 0,
        planning_reasoning_lift: result.categorySummary.find((item) => item.category === "planning")?.looseQualityLift ?? 0,
        planning_control_lift: result.categorySummary.find((item) => item.category === "planning")?.qualityLift ?? 0,
        recovery_reasoning_lift: result.categorySummary.find((item) => item.category === "recovery")?.looseQualityLift ?? 0,
        recovery_control_lift: result.categorySummary.find((item) => item.category === "recovery")?.qualityLift ?? 0,
        coding_reasoning_lift: result.categorySummary.find((item) => item.category === "coding")?.looseQualityLift ?? 0,
        coding_control_lift: result.categorySummary.find((item) => item.category === "coding")?.qualityLift ?? 0,
        medium_reasoning_lift: result.difficultySummary.find((item) => item.difficulty === "medium")?.looseQualityLift ?? 0,
        medium_control_lift: result.difficultySummary.find((item) => item.difficulty === "medium")?.qualityLift ?? 0,
        hard_reasoning_lift: result.difficultySummary.find((item) => item.difficulty === "hard")?.looseQualityLift ?? 0,
        hard_control_lift: result.difficultySummary.find((item) => item.difficulty === "hard")?.qualityLift ?? 0,
  ...
  if (!SKIP_CLEANUP) {
    progressJSON({ type: "cleanup_start", scope: "semantic_quality_matrix" })
    await rm(benchmarkRoot, { recursive: true, force: true }).catch(() => {})
    await rm(instanceRoot, { recursive: true, force: true }).catch(() => {})
    progressJSON({ type: "cleanup_done", scope: "semantic_quality_matrix" })
  } else {
    progressJSON({ type: "cleanup_skipped", scope: "semantic_quality_matrix" })
  }
}
