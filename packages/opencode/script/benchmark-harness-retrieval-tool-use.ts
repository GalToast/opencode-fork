import { runRetrievalToolUseBenchmark, classifyRetrievalToolUseLift, retrievalToolUsePolicyHint } from "../src/harness/retrieval-tool-use-benchmark"
...
async function withWorkspace<T>(fn: (prepareWorkspace: (scenarioID: string) => Promise<string>) => Promise<T>) {
  const root = await mkdtemp(path.join(os.tmpdir(), "opencode-retrieval-tool-use-"))
...
const profile = classifyRetrievalToolUseLift(result)

if (JSON_OUTPUT) {
  console.log(JSON.stringify({ ...result, profile, recommended_use: retrievalToolUsePolicyHint(profile) }, null, 2))
} else {
  console.log("Retrieval tool-use benchmark")
  ...
  console.log(`recommended_use: ${retrievalToolUsePolicyHint(profile)}`)
  console.table(
