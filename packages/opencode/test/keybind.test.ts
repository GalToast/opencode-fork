...
import { Keybind, type KeybindInfo } from "../src/util/keybind"

describe("Keybind.toString", () => {
  test("should convert simple key to string", () => {
    const info: KeybindInfo = { ctrl: false, meta: false, shift: false, leader: false, name: "f" }
    expect(Keybind.toString(info)).toBe("f")
  })

  test("should convert ctrl modifier to string", () => {
    const info: KeybindInfo = { ctrl: true, meta: false, shift: false, leader: false, name: "x" }
    expect(Keybind.toString(info)).toBe("ctrl+x")
  })

  test("should convert leader key to string", () => {
    const info: KeybindInfo = { ctrl: false, meta: false, shift: false, leader: true, name: "f" }
    expect(Keybind.toString(info)).toBe("<leader> f")
  })

  test("should convert multiple modifiers to string", () => {
    const info: KeybindInfo = { ctrl: true, meta: true, shift: false, leader: false, name: "g" }
    expect(Keybind.toString(info)).toBe("ctrl+alt+g")
  })

  test("should convert all modifiers to string", () => {
    const info: KeybindInfo = { ctrl: true, meta: true, shift: true, leader: true, name: "h" }
  ...
  test("should convert function key to string", () => {
    const info: KeybindInfo = { ctrl: false, meta: false, shift: false, leader: false, name: "f2" }
  ...
  test("should handle empty name", () => {
    const info: KeybindInfo = { ctrl: true, meta: false, shift: false, leader: false, name: "" }
    expect(Keybind.toString(info)).toBe("ctrl")
  })

  test("should handle only modifiers", () => {
    const info: KeybindInfo = { ctrl: true, meta: true, shift: true, leader: true, name: "" }
    expect(Keybind.toString(info)).toBe("<leader> ctrl+alt+shift")
  })

  test("should handle only leader with no other parts", () => {
    const info: KeybindInfo = { ctrl: false, meta: false, shift: false, leader: true, name: "" }
    expect(Keybind.toString(info)).toBe("<leader>")
  })

  test("should convert super modifier to string", () => {
    const info: KeybindInfo = { ctrl: false, meta: false, shift: false, super: true, leader: false, name: "z" }
    expect(Keybind.toString(info)).toBe("super+z")
  })

  test("should convert super+shift modifier to string", () => {
    const info: KeybindInfo = { ctrl: false, meta: false, shift: true, super: true, leader: false, name: "z" }
    expect(Keybind.toString(info)).toBe("super+shift+z")
  })

  test("should handle super with ctrl modifier", () => {
    const info: KeybindInfo = { ctrl: true, meta: false, shift: false, super: true, leader: false, name: "a" }
    expect(Keybind.toString(info)).toBe("ctrl+super+a")
  })

  test("should handle super with all modifiers", () => {
    const info: KeybindInfo = { ctrl: true, meta: true, shift: true, super: true, leader: false, name: "x" }
    expect(Keybind.toString(info)).toBe("ctrl+alt+super+shift+x")
  })

  test("should handle undefined super field (omitted)", () => {
    const info: KeybindInfo = { ctrl: true, meta: false, shift: false, leader: false, name: "c" }
...
describe("Keybind.match", () => {
  test("should match identical keybinds", () => {
    const a: KeybindInfo = { ctrl: true, meta: false, shift: false, leader: false, name: "x" }
    const b: KeybindInfo = { ctrl: true, meta: false, shift: false, leader: false, name: "x" }
    expect(Keybind.match(a, b)).toBe(true)
  })

  test("should not match different key names", () => {
    const a: KeybindInfo = { ctrl: true, meta: false, shift: false, leader: false, name: "x" }
    const b: KeybindInfo = { ctrl: true, meta: false, shift: false, leader: false, name: "y" }
    expect(Keybind.match(a, b)).toBe(false)
  })

  test("should not match different modifiers", () => {
    const a: KeybindInfo = { ctrl: true, meta: false, shift: false, leader: false, name: "x" }
    const b: KeybindInfo = { ctrl: false, meta: false, shift: false, leader: false, name: "x" }
    expect(Keybind.match(a, b)).toBe(false)
  })

  test("should match leader keybinds", () => {
    const a: KeybindInfo = { ctrl: false, meta: false, shift: false, leader: true, name: "f" }
    const b: KeybindInfo = { ctrl: false, meta: false, shift: false, leader: true, name: "f" }
    expect(Keybind.match(a, b)).toBe(true)
  })

  test("should not match leader vs non-leader", () => {
    const a: KeybindInfo = { ctrl: false, meta: false, shift: false, leader: true, name: "f" }
    const b: KeybindInfo = { ctrl: false, meta: false, shift: false, leader: false, name: "f" }
    expect(Keybind.match(a, b)).toBe(false)
  })

  test("should match complex keybinds", () => {
    const a: KeybindInfo = { ctrl: true, meta: true, shift: false, leader: false, name: "g" }
    const b: KeybindInfo = { ctrl: true, meta: true, shift: false, leader: false, name: "g" }
    expect(Keybind.match(a, b)).toBe(true)
  })

  test("should not match with one modifier different", () => {
    const a: KeybindInfo = { ctrl: true, meta: true, shift: false, leader: false, name: "g" }
    const b: KeybindInfo = { ctrl: true, meta: true, shift: true, leader: false, name: "g" }
    expect(Keybind.match(a, b)).toBe(false)
  })

  test("should match simple key without modifiers", () => {
    const a: KeybindInfo = { ctrl: false, meta: false, shift: false, leader: false, name: "a" }
    const b: KeybindInfo = { ctrl: false, meta: false, shift: false, leader: false, name: "a" }
    expect(Keybind.match(a, b)).toBe(true)
  })

  test("should match super modifier keybinds", () => {
    const a: KeybindInfo = { ctrl: false, meta: false, shift: false, super: true, leader: false, name: "z" }
    const b: KeybindInfo = { ctrl: false, meta: false, shift: false, super: true, leader: false, name: "z" }
    expect(Keybind.match(a, b)).toBe(true)
  })

  test("should not match super vs non-super", () => {
    const a: KeybindInfo = { ctrl: false, meta: false, shift: false, super: true, leader: false, name: "z" }
    const b: KeybindInfo = { ctrl: false, meta: false, shift: false, super: false, leader: false, name: "z" }
    expect(Keybind.match(a, b)).toBe(false)
  })

  test("should match undefined super with false super", () => {
    const a: KeybindInfo = { ctrl: true, meta: false, shift: false, leader: false, name: "c" }
    const b: KeybindInfo = { ctrl: true, meta: false, shift: false, super: false, leader: false, name: "c" }
    expect(Keybind.match(a, b)).toBe(true)
  })

  test("should match super+shift combination", () => {
    const a: KeybindInfo = { ctrl: false, meta: false, shift: true, super: true, leader: false, name: "z" }
    const b: KeybindInfo = { ctrl: false, meta: false, shift: true, super: true, leader: false, name: "z" }
    expect(Keybind.match(a, b)).toBe(true)
  })

  test("should not match when only super differs", () => {
    const a: KeybindInfo = { ctrl: true, meta: true, shift: true, super: true, leader: false, name: "a" }
    const b: KeybindInfo = { ctrl: true, meta: true, shift: true, super: false, leader: false, name: "a" }
    expect(Keybind.match(a, b)).toBe(false)
  })
})
