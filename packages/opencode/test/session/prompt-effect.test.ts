...
const FirstTool = Tool.define("first", {
  description: "Test tool used by prompt loop tests.",
...
function makeHttp() {
  ...
  const registryWithFirst = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      return ToolRegistry.Service.of({
        ...base,
        tools: (model, agent) =>
          base.tools(model, agent).pipe(
            Effect.flatMap((tools) =>
              Effect.promise(async () => {
                const first = await FirstTool.init({ agent })
                return [
                  ...tools,
                  {
                    id: FirstTool.id,
                    ...
                    formatValidationError: first.formatValidationError,
                  },
                ]
              }),
            ),
  ...
  return Layer.mergeAll(
    ...
    SessionPrompt.layer.pipe(
      ...
      Layer.provideMerge(registryWithFirst),
...
it.live("loop continues when assistant tool results complete without a finish reason", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
              ...
              choices: [
                {
                  delta: {
                    tool_calls: [
                      {
                        index: 0,
                        function: { arguments: JSON.stringify({ value: "first" }) },
                      },
                    ],
                  },
                },
...
it.live("background subtask preserves dispatcher metadata", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      ...
      expect(tool.state.output).toContain("Task is running in background")
      expect(tool.state.metadata).toBeDefined()
      expect(tool.state.metadata?.sessionId).toBeDefined()
      expect(tool.state.metadata?.status).toBe("completed")
...
it.live(
  "prompt submitted during an active error result starts the next run",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        const gate = defer<void>()
