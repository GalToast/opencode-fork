...
async function waitForPendingNext(count: number) {
  for (let i = 0; i < 20; i++) {
    const list = PermissionNext.list()
    if (list.length === count) return list
    await Bun.sleep(0)
  }
...
test("reply - resolves pending request even from sibling instance context", async () => {
  await using one = await tmpdir({ git: true })
  await using two = await tmpdir({ git: true })
  ...
  const ask = Instance.provide({
    ...
    fn: () =>
      PermissionNext.ask({
        ...
        metadata: {},
        always: ["C:/Users/HP/.codex/memories-cross-context/*"],
        ruleset: [],
      }),
  })
...
test("reply - sibling instance publishes replied event to original request instance", async () => {
  await using one = await tmpdir({ git: true })
  await using two = await tmpdir({ git: true })

  let seen: { requestID: string; sessionID: string; reply: PermissionNext.Reply } | undefined
  await Instance.provide({
    directory: one.path,
    fn: async () => {
  ...
  const ask = Instance.provide({
    ...
    fn: () =>
      PermissionNext.ask({
        ...
        metadata: {},
        always: ["C:/Users/HP/.codex/memories-cross-context-event/*"],
        ruleset: [],
      }),
  })
...
test("route reply - generated SDK resolves pending request through TUI-style fetch bridge", async () => {
  await using one = await tmpdir({ git: true })
  await using two = await tmpdir({ git: true })
  ...
  const ask = Instance.provide({
    ...
    fn: () =>
      PermissionNext.ask({
        ...
        metadata: {},
        always: ["C:/Users/HP/.codex/memories-route-cross-context/*"],
        ruleset: [],
      }),
  })
  ...
  const client = createOpencodeClient({
    ...
    fetch: (async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      url.searchParams.set("directory", two.path)
      const request = new Request(url, original)
      request.headers.set("x-opencode-directory", two.path)
      return Server.App().fetch(request)
    }) as typeof fetch,
  })

  const result = await client.permission.reply({
    requestID: onePending[0].id,
    reply: "always",
  })
