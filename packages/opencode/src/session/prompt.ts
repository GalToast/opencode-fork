...
import { SessionForeground } from "./foreground"
import { SessionMission } from "./mission"
import { LLM } from "./llm"
...
export namespace SessionPrompt {
  ...
  const steer = Instance.state(
    () => ({} as Record<string, number>),
    async (current) => {
      for (const key of Object.keys(current)) delete current[key]
    },
  )
  const harnessResultTaps = new Map<string, (event: any) => Promise<void>>()
  const harnessSessionResultTaps = new Map<string, string>()

  export function registerHarnessResultTap(id: string, handler: (event: any) => Promise<void>) {
    harnessResultTaps.set(id, handler)
  }

  export function unregisterHarnessResultTap(id: string) {
    harnessResultTaps.delete(id)
  }

  const emitHarnessResultTap = (id: string | undefined, event: Record<string, unknown>) =>
    Effect.sync(() => {
      if (!id) return
      const handler = harnessResultTaps.get(id)
      if (!handler) return
      void handler(event).catch(() => {})
    })

  const emitHarnessSessionTap = (sessionID: SessionID, event: Record<string, unknown>) =>
    emitHarnessResultTap(harnessSessionResultTaps.get(sessionID), event)

  function shellCommand(command: string) {
    const sh = Shell.preferred()
    const argsByShell: Record<string, string[]> = {
      nu: ["-c", command],
      fish: ["-c", command],
      zsh: ["-l", "-c", command],
      bash: ["-l", "-c", command],
    ...
    return [sh, ...(argsByShell[Shell.name(sh)] ?? ["-c", command])]
  }
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
      ...
      const prompt: (input: PromptInput) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.prompt")(
        function* (input: PromptInput) {
          const session = yield* sessions.get(input.sessionID)
          const rootSessionID = session.parentID ?? input.sessionID
          let steerMessageID = input.messageID
          let steerReceivedAt = 0
          const finishSteer = Effect.gen(function* () {
            if (!input.steer) return
            const current = steer()
            current[input.sessionID] = Math.max(0, (current[input.sessionID] ?? 1) - 1)
            const pending = current[input.sessionID]
            if (pending === 0) delete current[input.sessionID]
            const at = Date.now()
            const latencyMS = steerReceivedAt ? Math.max(0, at - steerReceivedAt) : undefined
            yield* Effect.promise(() =>
              SessionForeground.steer({
                sessionID: input.sessionID,
                rootSessionID,
                stage: "applied",
                at,
          ...
          const result = yield* Effect.gen(function* () {
            if (input.steer) {
              const current = steer()
              current[input.sessionID] = (current[input.sessionID] ?? 0) + 1
              const pending = current[input.sessionID]
              const at = Date.now()
              steerReceivedAt = at
              yield* Effect.promise(() =>
            ...
            steerMessageID = message.info.id
            if (input.resultTapID) harnessSessionResultTaps.set(input.sessionID, input.resultTapID)
            yield* emitHarnessResultTap(input.resultTapID, {
              type: "prompt.user_created",
              sessionID: input.sessionID,
            ...
            if (runner.busy) {
              yield* runner.ensureRunning(runLoop(input.sessionID)).pipe(Effect.exit, Effect.asVoid)
            }
            return yield* runner.ensureRunning(runLoop(input.sessionID))
          }).pipe(Effect.exit)
          yield* finishSteer
          if (input.resultTapID) harnessSessionResultTaps.delete(input.sessionID)
      ...
      const runLoop: (sessionID: SessionID) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.run")(
        function* (sessionID: SessionID) {
          ...
          while (true) {
            ...
            yield* emitHarnessSessionTap(sessionID, {
              type: "prompt.loop_step",
              sessionID,
              step,
            })
            ...
            const outcome: "break" | "continue" = yield* Effect.onExit(
              Effect.gen(function* () {
                ...
                yield* emitHarnessSessionTap(sessionID, {
                  type: "prompt.processor_start",
                  sessionID,
                  step,
                  messages: modelMsgs.length,
      ...
      const command = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
        ...
        if (shellMatches.length > 0) {
          const results = yield* Effect.promise(() =>
            Promise.all(
              shellMatches.map(async ([, cmd]) => (await Process.run(shellCommand(cmd), { nothrow: true })).stdout.toString()),
            ),
          )
          let index = 0
  ...
  export const PromptInput = z.object({
    ...
    priority: z.enum(["urgent", "steer", "normal", "background"]).optional(),
    resultTapID: z.string().optional(),
  ...
  export function isSteerPending(sessionID: string) {
    return (steer()[sessionID] ?? 0) > 0
  }
