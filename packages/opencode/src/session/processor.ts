    ...
    Effect.gen(function* () {
      ...
      const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
        ...
        const cleanup = Effect.fn("SessionProcessor.cleanup")(function* () {
          ...
          const parts = MessageV2.parts(ctx.assistantMessage.id)
          if (!ctx.assistantMessage.finish && parts.some((part) => part.type === "text" && part.text.length > 0)) {
            ctx.assistantMessage.finish = "stop"
          }
        ...
        const halt = Effect.fn("SessionProcessor.halt")(function* (e: unknown) {
          ...
          if (
            ...
            String(error.data?.message ?? "").includes("usage.inputTokens.total")
          ) {
        ...
        const process = Effect.fn("SessionProcessor.process")(function* (streamInput: LLM.StreamInput) {
          ...
          return yield* Effect.gen(function* () {
            ...
            if (ctx.assistantMessage.time.completed && !ctx.blocked && !ctx.assistantMessage.error && !aborted) {
              const hasResolvedToolPart = MessageV2.parts(ctx.assistantMessage.id).some(
                (part) =>
                  part.type === "tool" && (part.state.status === "completed" || part.state.status === "error"),
              )
              if (hasResolvedToolPart) return "continue"
            }
