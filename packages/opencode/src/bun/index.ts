...
import { Process, type Options as ProcessOptions } from "../util/process"

const log = Log.create({ service: "bun" })

async function run(cmd: string[], options?: ProcessOptions) {
  log.info("running", {
    cmd: [which(), ...cmd],
    ...options,
  })
...
const InstallFailedError = NamedError.create(
...
async function install(pkg: string, version = "latest") {
  ...
  const modExists = Filesystem.exists(mod)
