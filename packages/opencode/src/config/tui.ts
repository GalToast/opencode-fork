...
const state = Instance.state(async () => {
  ...
  projectFiles = Flag.OPENCODE_DISABLE_PROJECT_CONFIG ? [] : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)

  let result: TuiConfigInfo = {}
...
async function get() {
  return state().then((entry) => entry.config)
}

async function loadFile(filepath: string): Promise<TuiConfigInfo> {
...
async function load(text: string, configFilepath: string): Promise<TuiConfigInfo> {
  ...
  const normalized = (() => {
    ...
    const nestedTui = copy.tui
    if (!nestedTui || typeof nestedTui !== "object" || Array.isArray(nestedTui)) {
      delete copy.tui
      return {}
    }
    const tui = nestedTui as Record<string, unknown>
    delete copy.tui
    return {
      ...tui,
      ...copy,
