...
export async function migrateTuiConfig(input: MigrateInput) {
  ...
  for (const file of opencode) {
    ...
    if (!source) continue
    const data = parseLegacyTuiConfig(source)
    if (!data) continue
...
function parseLegacyTuiConfig(source: string) {
  const errors: JsoncParseError[] = []
  const value = parseJsonc(source, errors, { allowTrailingComma: true }) as unknown
  if (errors.length || !isRecord(value)) return undefined
  return value
}
