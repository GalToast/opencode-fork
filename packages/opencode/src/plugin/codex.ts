...
export function parseJwtClaims(token: string): IdTokenClaims | undefined {
  ...
  try {
    const claims = JSON.parse(Buffer.from(parts[1], "base64url").toString()) as unknown
...
function startOAuthServer(): { port: number; redirectUri: string } {
  if (oauthServer) {
  ...
  oauthServer = Bun.serve({
    ...
    fetch(req) {
      ...
      if (url.pathname === "/auth/callback") {
        ...
        void exchangeCodeForTokens(code, `http://localhost:${OAUTH_PORT}/auth/callback`, current.pkce)
          .then((tokens) => current.resolve(tokens))
          .catch((callbackError: unknown) => {
            current.reject(callbackError instanceof Error ? callbackError : new Error(String(callbackError)))
          })
...
export function CodexAuthPlugin(input: PluginInput): Promise<Hooks> {
  return Promise.resolve({
              ...
              async callback() {
                while (true) {
                  ...
                  if (response.ok) {
                    ...
                    const codexTokens = tokens as TokenResponse

                    return {
                      type: "success" as const,
                      refresh: codexTokens.refresh_token,
                      access: codexTokens.access_token,
                      expires: Date.now() + (codexTokens.expires_in ?? 3600) * 1000,
                      accountId: extractAccountId(codexTokens),
                    }
                  }
    ...
    "chat.headers": (chatInput, output) => {
      if (chatInput.model.providerID !== "openai") return Promise.resolve()
