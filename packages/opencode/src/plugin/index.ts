...
const state = Instance.state(async () => {
  const client = createOpencodeClient({
    ...
    // @ts-expect-error - fetch type incompatibility
  ...
  for (let plugin of plugins) {
    ...
    if (!plugin.startsWith("file://")) {
      ...
      plugin = await BunProc.install(pkg, version).catch((err: unknown) => {
    ...
    await import(plugin)
      .then(async (mod) => {
        ...
        for (const rawPlugin of Object.values(exports)) {
          if (typeof rawPlugin !== "function") continue
          const fn = rawPlugin as PluginInstance
