...
function taskProgressTitle(action: string | undefined, description?: string) {
  const suffix = description ? `: ${description}` : ""
  if (action === "status") return `Task status${suffix}`
  if (action === "wait") return `Waiting for task${suffix}`
  if (action === "cancel") return `Cancel task${suffix}`
  if (action === "pause") return `Pause task${suffix}`
  if (action === "resume") return `Resume task${suffix}`
  if (action === "escalate") return `Escalate task${suffix}`
  return description
}
...
function task(info: ToolProps<typeof TaskTool>) {
  ...
  const name = taskProgressTitle(action, desc) ?? `${agent} Task`
  inline({
    icon,
    title: name,
...
function skill(info: ToolProps<typeof SkillTool>) {
  const input = info.input as { name?: string; query?: string }
  inline({
    icon: "→",
    title: input.name ? `Skill "${input.name}"` : `Skill suggestions for "${input.query ?? "query"}"`,
  })
}
