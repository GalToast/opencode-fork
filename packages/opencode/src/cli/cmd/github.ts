...
type ExchangeTokenErrorResponse = {
  error?: string
}
      ...
      async fn() {
        {
          ...
          async function promptModel() {
            ...
            if (!providerData) throw new UI.CancelledError()
          ...
          async function installGitHubApp() {
            ...
            if (existingInstallation) return s.stop("GitHub app already installed")
            ...
            async function getInstallation(): Promise<GitHubAppInstallationResponse["installation"]> {
              const response = await fetch(
                `https://api.opencode.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
              const responseJson: unknown = await response.json()
              const data = responseJson as GitHubAppInstallationResponse
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const isWorkflowDispatchEvent = context.eventName === "workflow_dispatch"
        ...
        const { providerID: selectedProviderID, modelID: selectedModelID } = normalizeModel()
      ...
      try {
        ...
        let message: string | Error = error instanceof Error ? error : String(error)
        ...
        if (isUserEvent) {
          await createComment(`${message}${footer()}`)
          await removeReaction(commentType)
        }
      ...
      async function exchangeForAppToken(token: string) {
        ...
        if (!response.ok) {
          ...
          const responseJson = errorJson as ExchangeTokenErrorResponse
          throw new Error(
            `App token exchange failed: ${response.status} ${response.statusText} - ${responseJson.error ?? "unknown"}`,
          )
        }

        const successJson: unknown = await response.json()
        const responseJson = successJson as ExchangeTokenSuccessResponse
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      async function assertPermissions() {
        ...
        try {
          ...
          throw new Error(`Failed to check permissions for user ${actor}: ${errorMessage}`)
        }

        if (!["admin", "write"].includes(permission)) throw new Error(`User ${actor} does not have write permissions`)
      }

      async function addReaction(reactionCommentType?: "issue" | "pr_review") {
      ...
      async function removeReaction(reactionCommentType?: "issue" | "pr_review") {
        // Only called for non-schedule events, so triggerCommentId is defined
      ...
      async function createPR(base: string, branch: string, title: string, body: string): Promise<number | null> {
        ...
        try {
          ...
          console.log(`Failed to check for existing PR: ${error instanceof Error ? error.message : String(error)}`)
        }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/opencode-share/${title64}.png?model=${selectedProviderID}/${selectedModelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
