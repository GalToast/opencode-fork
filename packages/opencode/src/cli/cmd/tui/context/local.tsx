...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const agent = iife(() => {
      ...
      const firstAgentName = () => agents()[0]?.name ?? visibleAgents()[0]?.name ?? "build"
      const [agentStore, setAgentStore] = createStore<{
        current: string
      }>({
        current: firstAgentName(),
      })
      ...
      return {
        ...
        current() {
          const fallback = agents()[0] ?? visibleAgents()[0]
          return (
            agents().find((x) => x.name === agentStore.current) ??
            fallback ?? { name: firstAgentName(), model: undefined, mode: "primary", hidden: false }
          )
        },
        set(name: string) {
