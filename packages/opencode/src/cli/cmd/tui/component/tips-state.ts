...
export function deriveTipText(context?: "first" | "returning", seed?: string) {
  const pool = context === "first" ? TIPS.slice(0, 18) : TIPS
  const stableSeed = `${context ?? "default"}:${seed ?? "ambient"}`
  return pool[hashSeed(stableSeed) % pool.length] ?? pool[0] ?? ""
}
