...
export const { use: useFrecency, provider: FrecencyProvider } = createSimpleContext({
  name: "Frecency",
  init: () => {
    const frecencyPath = join(Global.Path.state, "frecency.jsonl")
    ...
    onMount(() => {
      void (async () => {
        ...
        if (sorted.length > 0) {
          ...
          void writeFile(frecencyPath, content).catch(() => {})
        }
      })()
    })
    ...
    function updateFrecency(filePath: string) {
      ...
      if (Object.keys(store.data).length > MAX_FRECENCY_ENTRIES) {
        ...
        const content = sorted.map(([entryPath, entry]) => JSON.stringify({ path: entryPath, ...entry })).join("\n") + "\n"
        void writeFile(frecencyPath, content).catch(() => {})
      }
    }

    return {
      getFrecency: (filePath: string) => calculateFrecency(store.data[resolve(process.cwd(), filePath)]),
      updateFrecency,
      data: () => store.data,
    }
  },
