...
function bootTrace(message: string) {
  const path = process.env["OPENCODE_TUI_BOOT_TRACE"]
  if (!path) return
  try {
...
function rendererScreenMode(): CliRendererConfig["screenMode"] {
  switch (process.env["OPENCODE_TUI_SCREEN_MODE"]) {
    case "main":
    case "main-screen":
      return "main-screen"
    case "split":
...
function rendererConfig(_config: TuiConfig.Info): CliRendererConfig {
  return {
    ...
    screenMode: rendererScreenMode(),
    testing: process.env["OPENCODE_TUI_SKIP_TERMINAL_SETUP"] === "1",
    targetFps: 60,
    gatherStats: false,
    exitOnCtrlC: false,
    useKittyKeyboard:
      process.env["OPENCODE_TUI_DISABLE_KITTY"] === "1"
        ? (false as unknown as CliRendererConfig["useKittyKeyboard"])
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  const ready = createMemo(() => pluginsReady() && sync.ready)
  createEffect(() => {
    bootTrace(`app:ready:${ready()}`)
  })
  TuiPluginRuntime.init(api)
    .catch((error) => {
      bootTrace(`app:plugins:error:${errorMessage(error)}`)
      console.error("Failed to load TUI plugins", error)
    })
    .finally(() => {
      bootTrace("app:plugins:done")
