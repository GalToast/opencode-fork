...
import { sessionPlanTrackerCommandOptions } from "./plan-tracker-command-options"
...
export function Session() {
  ...
  createEffect(() => {
    ...
    if (workspaceID) (sdk as { setWorkspace?: (workspaceID: string) => void }).setWorkspace?.(workspaceID)
  })
    ...
    >
      <box flexDirection="row">
        <box flexGrow={1} paddingBottom={1} paddingLeft={2} paddingRight={2} gap={1}>
          <Show when={session()}>
            <scrollbox
              ref={(r: import("@opentui/core").ScrollBoxRenderable) => (scroll = r)}
              viewportOptions={{
...
function summarizeTaskLanePart(
  ...
  const messages = sessionID ? (args.messagesBySession[sessionID] ?? []) : []
  return {
    sessionID,
    status: deriveResolvedTaskStatus({ part, messages }),
  }
}
...
function Task(props: ToolProps<typeof TaskTool>) {
  ...
  const lane = createMemo(() =>
    summarizeTaskLanePart(part, {
      messagesBySession: sync.data.message,
    }),
  )
  const taskSessionID = createMemo(() => lane().sessionID)
  const knownTaskLabel = createMemo(() => {
    const targetTaskID = props.input.task_id
    if (typeof targetTaskID !== "string" || !targetTaskID.trim()) return undefined
  ...
  const resolvedTaskStatus = createMemo(() => deriveResolvedTaskStatus({ part, messages: messages() }))
  ...
  const taskTitle = createMemo(() => {
    ...
    if (props.input.action === "status") return `Task status${suffix}`
    if (props.input.action === "wait") return `Waiting for task${suffix}`
    if (props.input.action === "cancel") return `Cancel task${suffix}`
    if (props.input.action === "pause") return `Pause task${suffix}`
    if (props.input.action === "resume") return `Resume task${suffix}`
    if (props.input.action === "escalate") return `Escalate task${suffix}`
    if (description) return `${Locale.titlecase(subagentType())} Task - ${description}`
    return `${Locale.titlecase(subagentType())} Task`
  })
