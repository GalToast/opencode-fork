  ...
  new Hono()
    ...
    .get(
      ...
      describeRoute({
        ...
        description: "Get a lightweight catalog of local tools and commands available to planning surfaces.",
        operationId: "experimental.capability.list",
        responses: {
          200: {
      ...
      async (c) => {
        const items = [
          { id: "read", kind: "tool", source: "tool", title: "read", hints: [] },
          { id: "write", kind: "tool", source: "tool", title: "write", hints: [] },
          { id: "task", kind: "tool", source: "tool", title: "task", hints: [] },
          { id: "init", kind: "command", source: "command", title: "init", hints: [] },
          { id: "review", kind: "command", source: "command", title: "review", hints: [] },
        ]
        const tools = items.filter((item) => item.kind === "tool").length
        const commands = items.filter((item) => item.kind === "command").length
        const mcp = items.length - tools - commands
        return c.json({
          counts: {
            total: items.length,
            tools,
