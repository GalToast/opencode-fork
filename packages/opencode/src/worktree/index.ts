...
const NotGitError = NamedError.create(
  "WorktreeNotGitError",
  z.object({
    message: z.string(),
  }),
...
const NameGenerationFailedError = NamedError.create(
...
const CreateFailedError = NamedError.create(
...
const StartCommandFailedError = NamedError.create(
...
const RemoveFailedError = NamedError.create(
...
const ResetFailedError = NamedError.create(
  ...
  async function runStartCommand(directory: string, cmd: string) {
    if (process.platform === "win32") {
      ...
      if (shellName === "pwsh" || shellName === "powershell") {
        return $`${shell} -NoProfile -Command ${cmd}`.nothrow().cwd(directory)
      }
...
async function makeWorktreeInfo(name?: string): Promise<z.infer<typeof Info>> {
...
async function createFromInfo(info: z.infer<typeof Info>, startCommand?: string) {
    ...
    try {
      Project.addSandbox(Instance.project.id, info.directory)
    } catch {
      // Best effort: ignore failures to keep behavior non-blocking.
    }
...
const create = fn(CreateInput.optional(), async (input) => {
...
const remove = fn(RemoveInput, async (input) => {
    if (Instance.project.vcs !== "git") {
      throw new NotGitError({ message: "Worktrees are only supported for git projects" })
    }
...
const reset = fn(ResetInput, async (input) => {
    if (Instance.project.vcs !== "git") {
      throw new NotGitError({ message: "Worktrees are only supported for git projects" })
    }
...
export type WorktreeInfo = z.infer<typeof Info>
export type WorktreeCreateInput = z.infer<typeof CreateInput>
export type WorktreeRemoveInput = z.infer<typeof RemoveInput>
export type WorktreeResetInput = z.infer<typeof ResetInput>
