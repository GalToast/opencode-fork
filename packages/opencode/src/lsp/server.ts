...
export namespace LSPServer {
  ...
  export const ESLint: Info = {
    ...
    async spawn(root) {
      const eslint = await Bun.resolve("eslint", Instance.directory)
      if (!eslint) return
  ...
  export const Oxlint: Info = {
    ...
    async spawn(root) {
      ...
      const resolveBin = async (target: string) => {
        ...
        if (Filesystem.exists(localBin)) return localBin
  ...
  export const Biome: Info = {
    ...
    async spawn(root) {
      ...
      if (Filesystem.exists(localBin)) bin = localBin
      if (!bin) {
      ...
      if (!bin) {
        const resolved = await Bun.resolve("biome", root)
        if (!resolved) return
        bin = BunProc.which()
  ...
  export const Ty: Info = {
    ...
    spawn(root) {
      ...
      for (const venvPath of potentialVenvPaths) {
        ...
        if (Filesystem.exists(potentialPythonPath)) {
      ...
      if (!binary) {
        for (const venvPath of potentialVenvPaths) {
          ...
          if (Filesystem.exists(potentialTyPath)) {
  ...
  export const Zls: Info = {
    ...
    async spawn(root) {
      ...
      if (!bin) {
        ...
        const release = (await releaseResponse.json()) as unknown as GitHubRelease
  ...
  export const JDTLS: Info = {
    ...
    async spawn(root) {
      ...
      const installed = Filesystem.exists(launcherDir)
      if (!installed) {
        if (Flag.OPENCODE_DISABLE_LSP_DOWNLOAD) return
      ...
      if (!Filesystem.exists(launcherJar)) {
  ...
  export const KotlinLS: Info = {
    ...
    async spawn(root) {
      ...
      const installed = Filesystem.exists(launcherScript)
      if (!installed) {
        if (Flag.OPENCODE_DISABLE_LSP_DOWNLOAD) return
  ...
  export const YamlLS: Info = {
    ...
    async spawn(root) {
      ...
      if (!binary) {
        ...
        const exists = Filesystem.exists(js)
        if (!exists) {
          if (Flag.OPENCODE_DISABLE_LSP_DOWNLOAD) return
  ...
  export const BashLS: Info = {
    ...
    root: () => Promise.resolve(Instance.directory),
    async spawn(root) {
