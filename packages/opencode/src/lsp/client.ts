...
export namespace LSPClient {
  ...
  export async function create(input: { serverID: string; server: LSPServer.Handle; root: string }) {
    ...
    const result = {
      ...
      notify: {
        async open(payload: { path: string }) {
          const resolvedPath = path.isAbsolute(payload.path) ? payload.path : path.resolve(Instance.directory, payload.path)
          const text = await Filesystem.readText(resolvedPath)
      ...
      async waitForDiagnostics(payload: { path: string }) {
        const normalizedPath = Filesystem.normalizePath(
          path.isAbsolute(payload.path) ? payload.path : path.resolve(Instance.directory, payload.path),
        )
