...
function sortPatternEntries<T>(patterns: Record<string, T>) {
  return pipe(
...
function all<T>(input: string, patterns: Record<string, T>) {
  const sorted = sortPatternEntries(patterns)
...
function allStructured<T>(input: { head: string; tail: string[] }, patterns: Record<string, T>) {
  const sorted = sortPatternEntries(patterns)
  let result: T | undefined
  for (const [pattern, value] of sorted) {
