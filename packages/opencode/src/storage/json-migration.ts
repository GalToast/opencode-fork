...
export namespace JsonMigration {
  ...
  type LegacyTimedRecord = JsonRecord & {
    time?: LegacyTime
  }
  ...
  function asProject(value: unknown): LegacyProject | undefined {
    return asRecord(value) as LegacyProject | undefined
  }

  function asSession(value: unknown): LegacySession | undefined {
    return asRecord(value) as LegacySession | undefined
  }

  function asTodoList(value: unknown): LegacyTodo[] | undefined {
    return Array.isArray(value) ? (value as LegacyTodo[]) : undefined
  }

  function asShare(value: unknown): LegacyShare | undefined {
    return asRecord(value) as LegacyShare | undefined
  }

  function asTimedRecord(value: unknown): LegacyTimedRecord | undefined {
    return asRecord(value) as LegacyTimedRecord | undefined
  }

  function omitKeys(value: unknown, keys: string[]): JsonRecord {
  ...
  export async function run(sqlite: Database, options?: Options) {
    ...
    async function read(files: string[], batchStart: number, end: number) {
      const count = end - batchStart
      const tasks = Array.from({ length: count }, () => Promise.resolve<unknown>(undefined))
      for (let i = 0; i < count; i++) {
        tasks[i] = Filesystem.readJson(files[batchStart + i])
      }
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        errs.push(`failed to read ${files[batchStart + i]}: ${String(result.reason)}`)
      }
      return items
    }

    function insert<T>(values: T[], write: (batch: T[]) => void, label: string) {
    ...
    for (let i = 0; i < projectFiles.length; i += batchSize) {
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        projectValues.push({
          ...
          sandboxes: (data.sandboxes ?? []) as ProjectInsert["sandboxes"],
          commands: data.commands as ProjectInsert["commands"],
        })
      }
    ...
    for (let i = 0; i < sessionFiles.length; i += batchSize) {
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        sessionValues.push({
          ...
          summary_diffs: (data.summary?.diffs ?? null) as SessionInsert["summary_diffs"],
          revert: data.revert as SessionInsert["revert"],
          permission: data.permission as SessionInsert["permission"],
          time_created: data.time?.created ?? now,
          time_updated: data.time?.updated ?? now,
    ...
    for (let i = 0; i < allMessageFiles.length; i += batchSize) {
      ...
      for (let j = 0; j < batch.length; j++) {
        const data = asTimedRecord(batch[j])
        if (!data) continue
      ...
      stats.messages += insert(
        values,
        (batchValues) => db.insert(MessageTable).values(batchValues).onConflictDoNothing().run(),
        "message",
      )
      step("messages", end - i)
    }
    ...
    for (let i = 0; i < partFiles.length; i += batchSize) {
      ...
      stats.parts += insert(
        values,
        (batchValues) => db.insert(PartTable).values(batchValues).onConflictDoNothing().run(),
        "part",
      )
      step("parts", end - i)
    }
    ...
    for (let i = 0; i < todoFiles.length; i += batchSize) {
      ...
      stats.todos += insert(
        values,
        (batchValues) => db.insert(TodoTable).values(batchValues).onConflictDoNothing().run(),
        "todo",
      )
      step("todos", end - i)
    }
    ...
    for (let i = 0; i < permFiles.length; i += batchSize) {
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        permValues.push({ project_id: projectID, data: data as PermissionInsert["data"] })
      }
