...
type OpenAICompatibleChatUsage = {
...
type OpenAICompatibleChatMessage = {
  ...
  tool_calls?: Array<OpenAICompatibleChatToolCall> | null
}

type OpenAICompatibleChatChoice = {
...
type OpenAICompatibleChatDeltaToolCall = {
  index: number
  id?: string | null
  function?: {
    name?: string | null
...
type OpenAICompatibleChatDelta = {
  role?: "assistant" | null
  content?: string | null
  reasoning_text?: string | null
  reasoning_opaque?: string | null
  tool_calls?: Array<OpenAICompatibleChatDeltaToolCall> | null
}

type OpenAICompatibleChatChunkSuccess = {
  id?: string | null
  created?: number | null
  model?: string | null
  choices: Array<{
    delta?: OpenAICompatibleChatDelta | null
    finish_reason?: string | null
  }>
  usage?: OpenAICompatibleChatUsage
}

type OpenAICompatibleChatChunk = OpenAICompatibleChatChunkSuccess | { error: unknown }
...
export class OpenAICompatibleChatLanguageModel implements LanguageModelV2 {
  ...
  private readonly chunkSchema: z.ZodType<OpenAICompatibleChatChunk>
  ...
  async doGenerate(
    ...
    const {
    ...
    } = await postJsonToApi<OpenAICompatibleChatResponse>({
      url: this.config.url({
      ...
      // eslint-disable-next-line @typescript-eslint/no-use-before-define
      successfulResponseHandler: createJsonResponseHandler<OpenAICompatibleChatResponse>(OpenAICompatibleChatResponseSchema),
  ...
  async doStream(
    ...
    const { responseHeaders, value: response } = await postJsonToApi<ReadableStream<ParseResult<OpenAICompatibleChatChunk>>>({
      url: this.config.url({
        path: "/chat/completions",
        modelId: this.modelId,
      }),
      ...
      body,
      failedResponseHandler: this.failedResponseHandler,
      successfulResponseHandler: createEventSourceResponseHandler<OpenAICompatibleChatChunk>(this.chunkSchema),
      abortSignal: options.abortSignal,
      fetch: this.config.fetch,
    })
    ...
    return {
      stream: response.pipeThrough(
        new TransformStream<ParseResult<OpenAICompatibleChatChunk>, LanguageModelV2StreamPart>({
          start(controller) {
          ...
          transform(chunk, controller) {
            ...
            if ("error" in value) {
              finishReason = "error"
              controller.enqueue({ type: "error", error: getErrorMessage(value.error) })
              return
            }
          ...
          flush(controller) {
            ...
            for (const toolCall of toolCalls.filter((item) => !item.hasFinished)) {
              controller.enqueue({
                type: "tool-input-end",
                id: toolCall.id,
              })
