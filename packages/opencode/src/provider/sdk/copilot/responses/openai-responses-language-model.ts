...
type OpenAIResponsesOutputItem =
  | z.infer<typeof webSearchCallItem>
  | z.infer<typeof fileSearchCallItem>
  | z.infer<typeof codeInterpreterCallItem>
  | z.infer<typeof localShellCallItem>
  | z.infer<typeof imageGenerationCallItem>
  | {
      type: "message"
      role: "assistant"
      id: string
      content: Array<{
        type: "output_text"
        text: string
        logprobs?: z.infer<typeof LOGPROBS_SCHEMA> | null
        annotations: Array<
          | {
              type: "url_citation"
              start_index: number
...
const fail = openaiFailedResponseHandler as unknown as ResponseHandler<APICallError>
...
export class OpenAIResponsesLanguageModel implements LanguageModelV2 {
  ...
  async doGenerate(
    ...
    const out = (await postJsonToApi<OpenAIResponsesResponse>({
      url,
      headers: combineHeaders(this.config.headers(), options.headers),
      body,
      failedResponseHandler: fail,
      successfulResponseHandler: createJsonResponseHandler<OpenAIResponsesResponse>(
        z.object({
          id: z.string(),
          created_at: z.number(),
          error: z
  ...
  async doStream(
    ...
    const out = (await postJsonToApi<ReadableStream<ParseResult<z.infer<typeof openaiResponsesChunkSchema>>>>({
      url: this.config.url({
        path: "/responses",
        modelId: this.modelId,
      }),
      ...
      abortSignal: options.abortSignal,
      fetch: this.config.fetch,
    })) as OpenAIResponsesStreamResult
    const { responseHeaders, value: response } = out

    const makeId = () => this.config.generateId?.() ?? generateId()
