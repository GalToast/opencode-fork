...
$useBunRunner = ($launchMode -eq "source" -and $bunCmd -and (Test-Path $entryPath)) -or
  (($launchMode -ne "binary") -and $bunCmd -and (Test-Path $entryPath) -and (-not $binaryExists))
...
if ($useBunRunner) {
  ...
  # Compile from the package root so Bun sees the package tsconfig/paths, while
  # OPENCODE_CALLER_CWD keeps the user's launch directory as the workspace.
